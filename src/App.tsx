...
if (!socket) return;
socket.emit('host', { duration });
};

const handleSetMeters = (meters: number) => {
if (!socket) return;
socket.emit('host', { meters });
};

const handleSetLanes = (lanes: number) => {
if (!socket) return;
socket.emit('host', { lanes });
};
...
if (!socket) return;
socket.emit('host', { mode, invitedUsers });
};
