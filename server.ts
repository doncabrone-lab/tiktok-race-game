...
async function startServer() {
  ...
  const TIKTOK_USERNAME = (process.env.TIKTOK_USERNAME || 'patronizzle').replace(/^@/, '').trim();
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('host:set_meters', (data: { meters: number }) => {
  ...
  // TikTok LIVE -> chat / likes / gifts -> GameEngine
  // ==========================================
  ...
  const connectToTikTok = () => {
    ...
    console.log(`[TikTok] Connecting to Live Room of @${TIKTOK_USERNAME}...`);
    ...
    connection
      ...
      .then((state) => {
        ...
        console.log(`[TikTok] CONNECTED @${TIKTOK_USERNAME} room=${tiktokRoomId}`);
      })
      .catch((err) => {
        tiktokConnected = false;
        tiktokRoomId = null;
        console.error(`[TikTok] CONNECT FAILED @${TIKTOK_USERNAME}:`, err?.message || err);
        scheduleTikTokReconnect(30000);
      });

    connection.on('chat', (data: any) => {
      const username = String(data?.uniqueId || data?.nickname || 'Spectator').replace(/^@/, '');
      const message = String(data?.comment || '').trim();
      if (!message) return;
      const isBroadcaster = username.toLowerCase() === TIKTOK_USERNAME.toLowerCase();
      console.log(`[TikTok] CHAT @${username}: ${message}`);
      void processChatMessage(username, message, isBroadcaster).catch((err) => {
        console.error(`[TikTok] CHAT PROCESS ERROR @${username}:`, err?.message || err);
      });
    });
    ...
    connection.on('like', (data: any) => {
      ...
      // Cap one websocket event to 10 game taps so a large TikTok like burst
      // cannot freeze the game loop.
    ...
    connection.on('gift', (data: any) => {
      ...
      // Combo gifts are emitted multiple times while the combo is active.
      // Process the final event only, matching the connector behavior already
      // used by the GitHub deployment.
      ...
      console.log(`[TikTok] GIFT @${username}: ${giftName} x${repeatCount} diamonds=${diamondCount}`);

      gameEngine.handleGift(username, giftName, repeatCount, undefined, {
        coinValue,
        diamondCount,
        repeatCount,
      });
    ...
    connection.on('streamEnd', () => {
      tiktokConnected = false;
      tiktokRoomId = null;
      console.log(`[TikTok] LIVE ended for @${TIKTOK_USERNAME}. Retrying in 30s...`);
      scheduleTikTokReconnect(30000);
    });
    ...
    connection.on('disconnected', () => {
      tiktokConnected = false;
      tiktokRoomId = null;
      console.log(`[TikTok] Disconnected from @${TIKTOK_USERNAME}. Retrying in 10s...`);
